...
const Faq = () => {
    ...
    return (
        <Grid container>
            ...
            <Grid item xs={12}>
                <Typography className={classes.typography}>FR: Forest Reserve <br />
                GR: Game Reserve <br />
                NP: National Park <br />
                WS: Wildlife Sanctuary</Typography>
            </Grid> 
            ...
            <Grid item xs={12}>
                ...
                <p><u>Haplotypes without a linked peer-reviewed journal article.</u> Some sequences may not yet be reported in a peer-reviewed journal, in which case the reference would be listed as “unpublished paper.”</p>

                A subsequent scientific paper may report on a haplotype sequence identical to one already listed in the Lion Localizer database, but without provenance, associated GenBank number, and/or a link to a peer-reviewed journal article. 
                In such cases, information from the newly published paper would be used to fill in the incomplete information for the haplotype in the Lion Localizer, with a link established to the new publication.
                </Typography>
            </Grid> 
            ...
            <Grid item xs={12}>
                <Typography className={classes.typography}>The Lion Localizer is designed to handle mitochondrial DNA sequences with unambiguous character states (A, C, G or T).
                <p>A very small proportion of published sequences in Genbank include one or more nucleotide sites with “N” as the character state (unknown nucleotide), within the region examined by the Lion Localizer. 
                Because these can represent low quality or low coverage of reads, we excluded this small proportion of GenBank entries from the Lion Localizer database, to minimize the use of sequence data that is sometimes
                not as reliable as published sequences with only unambiguous character states.</p>
                
                <b>Heteroplasmy</b> is defined as the presence, within a cell or individual, of more than
                one distinct sequence among organellar genomes. This leads to polymorphisms
                among DNA reads, producing ambiguous character states in sequence data that is
                otherwise of high quality.

                <p><b>Indels</b> are insertion-deletion variants that show up as gaps or additional bases in the
                sequence when it is aligned to similar sequences</p>

                For those using the Lion Localizer, we provide the following guidelines should indels
                or heteroplasmy be putatively detected at a site within the region of mtDNA used as
                input by the software:

                <p>• If there is a large batch of samples, and the sample with the indel or heteroplasmy is
                not critical, then consider removing the sample from further consideration.</p>

                • If the sample is critical or additional sequencing is being conducted, then verify that
                the indel or heteroplasmy is real and not an artifact due to a technical issue. For
                example, the DNA can be re-extracted if contamination after extraction is suspected.
                If Sanger sequencing is being conducted, it would be important to verify the indel or
                heteroplasmy by sequencing the mtDNA in both directions, and/or conducting the
                sequencing reactions several times, and also verifying that surrounding sequences can
                be identified unambiguously with high confidence.
                
                <p>• If the sample is critical, and the sequence is confirmed as containing a single
                heteroplasmic site, then both nucleotides present at the site can be coded and entered
                separately as queries against the Lion Localizer. For example, if the sequence
                contains:</p>

                …ACTGA<b>R</b>GTCAA… where R indicates that some reads contain A and other reads
                contain G at a nucleotide position.
                ...
                <p>• For a sequence that has an indel compared to the cytochrome b sequences in the Lion
                Localizer database, it may be preferable for the sequence with an indel to be excluded
