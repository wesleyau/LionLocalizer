...
def sequenceCsv(request):
    ...
    prompt = {
        "order": "VERY IMPORTANT: Sequences upload should only be 4 columns. Order of the columns in the CSV should be haplotype_id, cytB, lastEditedBy, and lastEditNotes.  lastEditBy and lastEditNotes can be empty"
    }
...
def locationCsv(request):
    ...
    prompt = {
        "order": "VERY IMPORTANT: locations upload should only be 10 columns - Order of the columns in the CSV should be id, locationName, lon, lat, locationType, matchnNotes, locality, accuracy, lastEditedBy, and lastEditNotes -  lastEditBy and lastEditNotes can be empty"
    }
...
def publicationCsv(request):
    ...
    prompt = {
        "order": "VERY IMPORTANT: publications upload should only be 5 columns - Order of the columns in the CSV should be id, author, paperurl, lastEditedBy, and lastEditNotes.  lastEditBy and lastEditNotes can be empty"
    }
...
def lochappubCsv(request):
    ...
    prompt = {
        "order": "VERY IMPORTANT: lochappub upload should only be 6 columns - Order of the columns in the CSV should be id, location, haplotype, author, lastEditedBy, and lastEditNotes.  lastEditBy and lastEditNotes can be empty"
    }
...
def lhpindividualCsv(request):
    ...
    prompt = {
        "order": "VERY IMPORTANT: lhpindividual upload should only be 6 columns - Order of the columns in the CSV should be id, LHP, genBankAccession, numLions, lastEditedBy, and lastEditNotes.  lastEditBy and lastEditNotes can be empty"
    }
...
def getCsv(request):
    ...
    for lhpi in LHPIndividual.objects.all():
        try:
            returnArr = [
                lhpi.pk,
                lhpi.genBankAccession,
                lhpi.numLions,
                lhpi.datetimeModified,
                lhpi.lastEditedBy,
                lhpi.lastEditNotes,
            ]
            lhp = LocHapPub.objects.get(
                pk=lhpi.LHP_id
            )  # should actually only have one item
            returnArr.extend([lhp.pk, lhp.lastEditedBy, lhp.lastEditNotes])
            loc = Locations.objects.get(
                pk=lhp.location.pk
            )  # should actually only have one item
            returnArr.extend(
                [
                    ...
                    loc.locality,
                    loc.accuracy,
                    loc.datetimeModified,
                    loc.lastEditedBy,
                    loc.lastEditNotes,
                ]
            )
            pub = Publications.objects.get(
                pk=lhp.author.pk
            )  # should actually only have one item
            returnArr.extend(
                [
